...
\begin{equation*}
  \begin{aligned}
    ...
    &= \FIRST(\{\T{num} \Seq \T{minus}, \T{num}, \T{var} \Seq \T{minus}, \T{variable}\})\\
    &= \{\T{num}, \T{var}\}
  \end{aligned}
\end{equation*}
...
\begin{equation*}
  \begin{aligned}
    ...
    \FIRST(\NT{Expr} \Spc \T{to} \Spc \T{var}) &= \FIRST(\FIRST(\NT{Expr}) \Seq \FIRST(\T{to}) \Seq \FIRST(\T{variable}))\\
    &= \FIRST(\{\T{num}, \T{var}\} \Seq \{\T{to}\} \Seq \{\T{variable}\})\\
    &= \FIRST(\{\T{num} \Seq \T{to} \Seq \T{var}, \T{variable} \Seq \T{to} \Seq \T{variable}\})\\
    &= \{\T{num}, \T{var}\}\\
...
\begin{equation*}
  \begin{aligned}
    ...
    &= \FIRST(\{\T{num} \Seq \T{seq}, \T{var} \Seq \T{seq}, \T{print} \Seq \T{seq}, \T{return} \Seq \T{seq}, \T{num}, \T{variable}, \T{print}, \T{return}\})\\
    &= \{\T{num}, \T{var}, \T{print}, \T{return}\}\\
  \end{aligned}
\end{equation*}
...
\begin{equation*}
  \begin{aligned}
    \FIRST(\FIRST(\NT{Expr} \Spc \T{to} \Spc \T{var}) \Seq \FOLLOW(\NT{Stmt})) &= \FIRST(\{\T{num}, \T{variable}\} \Seq \{\T{seq}, \EOF\})\\
    &= \FIRST(\{\T{num} \Seq \T{seq}, \T{var} \Seq \T{seq}, \T{num} \Seq \EOF, \T{variable} \Seq \EOF \})\\
    &= \{\T{num}, \T{var}\}\\
...
\begin{equation*}
  \begin{aligned}
    ...
    &= \FIRST(\{\T{minus}, \T{to}, \T{minus} \Seq \T{seq}, \T{minus} \Seq \EOF\})\\
    &= \{\T{minus}\}\\
...
\begin{equation*}
  \begin{aligned}
    ...
    &= \{\T{num}\}\\
    \FIRST(\FIRST(\T{var}) \Seq \FOLLOW(\NT{Primary})) &= \FIRST(\{\T{variable}\} \Seq \{\T{minus}, \T{to}, \T{seq}, \EOF\})\\
    &= \FIRST(\{\T{var} \Seq \T{minus}, \T{variable} \Seq \T{to}, \T{variable} \Seq \T{seq}, \T{variable} \Seq \EOF\})\\
    &= \{\T{var}\}
  \end{aligned}
\end{equation*}
...
\begin{algorithm}
  ...
  function recognizeStmt()
    return when lookahead
      $\T{num}, \T{var}$ $\rightarrow$ recognizeExpr() $\&\&$ recognizeTerminal($\T{to}$) $\&\&$ recognizeTerminal($\T{to}$)
...
\begin{algorithm}
  ...
  function recognizeStmt()
    switch lookahead
      case $\T{num}$:
      case $\T{var}$:
        return recognizeExpr() $\&\&$ recognizeTerminal($\T{to}$) $\&\&$ recognizeTerminal($\T{to}$)
      case $\T{print}$:
